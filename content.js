...
(() => {
  ...
  const READING_HIGHLIGHT_NAME = LectoroConstants.UI_CLASSES.READING_SENTENCE_HIGHLIGHT;
  ...
  function getIcon() {
    ...
    iconEl.appendChild(createToolbarButton("translate", SVG.TRANSLATE, "Translate", onIconClick));
  ...
  function showIcon(rect) {
    ...
    const { top: selTop, bottom: selBottom, left: selLeft, right: selRight, height } = rect;

    let bestX = mx + GAP;
    let bestY = my - ICON_H / 2;

    function overlapsSelection(ix, iy) {
      return !(ix > selRight || ix + ICON_W < selLeft || iy > selBottom || iy + ICON_H < selTop);
    }

    if (bestX + ICON_W <= vpW && !overlapsSelection(bestX, bestY)) {
      // good – right of mouse
    } else if (mx - GAP - ICON_W >= 0 && !overlapsSelection(mx - GAP - ICON_W, bestY)) {
      bestX = mx - GAP - ICON_W;
  ...
  const READING_BLOCK_SELECTOR = "p,div,li,blockquote,pre,td,th,h1,h2,h3,h4,h5,h6,article,section";
  ...
  function getSelectedTextSegments(range) {
    ...
    for (const node of nodes) {
      ...
      if (segments.length > 0 && block && previousBlock && block !== previousBlock) {
        fullText += "\n";
      }
    ...
    if (isEnd) {
      for (let i = segments.length - 1; i >= 0; i -= 1) {
        ...
        if (offset > segment.globalStart) {
          ...
          return {
            node: segment.node,
            offset: segment.nodeStart + Math.max(0, local - segment.globalStart),
          };
        }
      }
    } else {
  ...
  function buildReadingFragments(range, fallbackText) {
    try {
      ...
      return fallbackText ? [{ text: fallbackText, range: range.cloneRange() }] : [];
    }
  }
  ...
  function getReadingSafetyTimeout(text, rate) {
    ...
    const estimatedMs = (wordCount / Math.max(0.75, Number(rate) * 1.5 || 1.5)) * 1000;
  ...
  function speakSelectedText(text, lang, settings, session, onEnd) {
    ...
    try {
      ...
      utter.rate = Math.max(
        0.1,
        Math.min(10, Number(settings.speechRate) || DEFAULT_TTS.speechRate),
      );
      const volume =
        settings.ttsVolume !== undefined ? Number(settings.ttsVolume) : DEFAULT_TTS.ttsVolume;
    ...
    const finish = (failed = false) => {
      ...
      if (readingMonitorTimer === monitorTimer) readingMonitorTimer = null;
    ...
    safetyTimer = setTimeout(() => finish(true), getReadingSafetyTimeout(text, utter.rate));
    ...
    monitorTimer = setInterval(() => {
      ...
      if (synthesisSpeaking || (!hasStarted && window.speechSynthesis.pending)) {
        hasStarted = true;
        idleSince = 0;
        return;
      }
    ...
    const cancelGap = 100;
    const startDelay = Math.max(0, cancelGap - (Date.now() - lastSpeechCancelAt));
    const startSpeech = () => {
      if (readingStartTimer === startTimer) readingStartTimer = null;
      startTimer = null;
  ...
  async function startSelectedTextReading(fragments, lang, session) {
    ...
    const start = (settings) => {
      ...
      const readNext = () => {
        ...
        clearSentenceHighlight();
        try {
          if (fragment.range && typeof CSS !== "undefined" && CSS.highlights) {
            CSS.highlights.set(READING_HIGHLIGHT_NAME, new Highlight(fragment.range));
          }
        } catch (error) {
          console.warn("[Lectoro] Fragment highlight failed:", error);
        }

        speakSelectedText(fragment.text, lang, settings, session, () => {
          index += 1;
          readNext();
        });
      };

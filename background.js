...
async function uploadPendingScreenshots(words, logLabel) {
  for (const word of words) {
    ...
    try {
      ...
      const savedPath =
        uploaded?.relativePath || uploaded?.path || uploaded?.url;
      if (savedPath) {
      ...
      console.warn(
        `[Lectoro] R2 upload during ${logLabel} error:`,
        uploadError
      );
    }
  }
  ...
  syncInFlight = (async () => {
    ...
    if (entries.length > 0) {
      ...
      await uploadPendingScreenshots(
        upsertEntries.map((entry) => entry.word),
        "sync"
      );
...
async function fullSync({ pull = true } = {}) {
  ...
  const remoteMap = new Map(
    remoteWords.map((word) => [wordIdentity(word), word])
  );
  ...
  return new Promise((resolve, reject) => {
    const callback = (dataUrl) => {
      if (chrome.runtime.lastError || !dataUrl) {
        reject(
          chrome.runtime.lastError || new Error("No tab image available.")
        );
      } else {
        resolve(dataUrl);
      }
...
const MESSAGE_HANDLERS = Object.freeze({
  ...
  [MSG.GET_FIREBASE_TOKEN]: async (message) => {
    try {
      return {
        token: await getSingleFlightToken(!!message.forceRefresh),
      };
    } catch (error) {
  ...
  [MSG.FIRESTORE_DELETE_BATCH]: async (message) => {
    if (!Array.isArray(message.words))
      throw new Error("Missing words payload.");
  ...
  [MSG.GEMINI_UPLOAD_CARD_IMAGE]: async (message) => ({
    ok: true,
    result: await GeminiProxy.uploadCardImage(
      message.wordId,
      message.imageBase64,
      message.contentType
    ),

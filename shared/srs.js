...
const SRS = {
    ...
    update(sr = {}, grade = 2, now = Date.now()) {
        ...
        reps = reps || step || 0;
        easeFactor = Number.isFinite(easeFactor) && easeFactor >= MIN_EASE_FACTOR ? easeFactor : DEFAULT_EASE_FACTOR;
        ...
        if (isFail) {
            ...
            easeFactor = Math.max(MIN_EASE_FACTOR, Math.round((easeFactor - 0.2) * 100) / 100);
            lapses += 1;
            reps = 0;
            ...
            if (interval < INTRA_DAY_LEARN_INTERVAL_DAYS || reps === 0) {
                ...
                if (reps >= 4) {
                    easeFactor = Math.min(MAX_EASE_FACTOR, Math.round((easeFactor + 0.05) * 100) / 100);
                }
            }
        }

...
(() => {
    ...
    function positionTooltip(rect, preferredPosition = "top") {
        ...
        const viewportWidth = inFullscreen
            ? window.innerWidth
            : document.documentElement.clientWidth;
        const viewportHeight = inFullscreen
            ? window.innerHeight
    ...
    function rememberScreenshotContext(rect, anchorOverride = null) {
        ...
        if (
            anchorOverride &&
            anchorOverride.nodeType === Node.ELEMENT_NODE &&
            !isOwnUI(anchorOverride)
        ) {
    ...
    function getSharedContainerBonus(anchorElement, media) {
        ...
        for (let depth = 0; ancestor && depth < 20; depth += 1) {
            if (
                ...
                break;
            if (ancestor.contains?.(media))
                return Math.max(0, 5_000 - depth * 150);
    ...
    function getVisibleMediaCandidates() {
        ...
        return mediaElements
            ...
            .filter(({ media, rect, visibleArea }) => {
                ...
                const mediaLabel =
                    `${media.className || ""} ${media.id || ""} ${media.getAttribute?.("alt") || ""}`.toLowerCase();
            ...
            .map(({ media, rect, visibleRatio, visibleArea }) => {
                ...
                const sharedBonus = getSharedContainerBonus(
                    anchorElement,
                    media,
                );
                const isDirectShared =
                    sharedBonus > 0 ||
                    (anchorRoot && mediaRoot && anchorRoot === mediaRoot);

                let score =
                    dist + (1 - Math.min(1, visibleRatio)) * 250 - sharedBonus;
                if (anchorRoot && mediaRoot && anchorRoot === mediaRoot)
                    score -= 50_000;
                if (anchorElement && media.contains(anchorElement))
                    score -= 60_000;
                if (
                    anchorElement &&
                    anchorElement.parentElement?.contains(media)
                )
                    score -= 80_000;
    ...
    function renderScaledDataUrl(sourceWidth, sourceHeight, draw) {
        ...
        const scale = Math.min(
            SCREENSHOT_MAX_PX / sourceWidth,
            SCREENSHOT_MAX_PX / sourceHeight,
            1,
        );
    ...
    function extractImageSourceUrl(media) {
        ...
        if (target instanceof HTMLImageElement) {
            ...
            if (target.getAttribute?.("src"))
                candidates.push(target.getAttribute("src"));
        ...
        for (let candidate of candidates) {
            ...
            if (/^data:image\/(svg|gif);base64,R0lGOD/i.test(candidate))
                continue;
        ...
        return target instanceof HTMLImageElement
            ? target.currentSrc || target.src || null
            : null;
    }
    ...
    async function captureMediaScreenshot(media) {
        ...
        if (globalThis.LectoroNetflixAdapter?.isPage?.()) {
            const netflixImg =
                await globalThis.LectoroNetflixAdapter.captureReviewImage(
                    media,
                );
        ...
        if (media instanceof HTMLVideoElement) {
            ...
            if (media.poster) {
                const drawn = await drawFromDataUrl(
                    await requestImageDataUrl(media.poster),
                );
    ...
    function buildDictionaryDetailsHtml(dictionary, srcLang, targetLang) {
        if (
            !dictionary ||
            !Array.isArray(dictionary.senses) ||
            !dictionary.senses.length
        )
            return "";
        const P = PREFIX;
        const seen = new Set();
        ...
        return `<section class="${P}dictionary-details" aria-label="Dictionary details">${definition ? `<div class="${P}dictionary-definition-section"><div class="${P}dictionary-caption">Definition</div>${definition}</div>` : ""}${synonyms}${examples.length ? `<div class="${P}dictionary-caption">Examples</div>` : ""}${examples
            .slice(0, 3)
    ...
    function buildSaveFooterHtml(
        ...
        const P = PREFIX;
        const saveKeyHtml = saveKeyHint
            ? `<kbd class="${P}key-hint">${escapeHtml(saveKeyHint)}</kbd>`
            : "";
        const aiKeyHtml = aiKeyHint
            ? `<kbd class="${P}key-hint">${escapeHtml(aiKeyHint)}</kbd>`
            : "";
    ...
    function splitIntoWordSpans(el, wordClass) {
        ...
        if (
            childNodes.some(
                (n) => n.nodeName === "BR" || n.nodeType === Node.ELEMENT_NODE,
            )
        ) {
            el.innerHTML = "";
        ...
        for (const el of els) {
            ...
            if (subContainer) {
                ...
                if (
                    ...
                    !target.querySelector(`.${wordClass}`)
                ) {
                    splitIntoWordSpans(target, wordClass);
                    const newlyFound = Array.from(
                        document.elementsFromPoint(x, y),
                    ).find((n) => n.classList?.contains(wordClass));
    ...
    function getVideo() {
        return (
            globalThis.LectoroPlayerRegistry?.getVideo?.() ??
    ...
    const QT = {
        ...
        geminiExplainSentence: (s, tgt, ctx = null, opts = {}) =>
            SharedTranslatorService.explainSentence(s, tgt, ctx, opts),

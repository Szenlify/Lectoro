...
(() => {
  ...
  function rememberScreenshotContext(rect, anchorOverride = null) {
    ...
    if (
      ...
      anchorElement = document.elementsFromPoint(x, y).find((element) => !isOwnUI(element)) || null;
    }
    ...
    return mediaElements
      ...
      .map(({ media, rect, visibleRatio, visibleArea }) => {
        ...
        const isDirectShared =
          sharedBonus > 0 || (anchorRoot && mediaRoot && anchorRoot === mediaRoot);
  ...
  function extractImageSourceUrl(media) {
    ...
    const imgChild =
      media instanceof HTMLImageElement ? media : media.querySelector?.("img, image");
  ...
  async function captureMediaScreenshot(media) {
    ...
    if (globalThis.LectoroNetflixAdapter?.isPage?.()) {
      const netflixImg = await globalThis.LectoroNetflixAdapter.captureReviewImage(media);
  ...
  async function captureContextScreenshot() {
    if (globalThis.LectoroNetflixAdapter?.isPage?.()) {
      ...
      const netflixImage = await globalThis.LectoroNetflixAdapter.captureReviewImage(video);
  ...
  function buildDictionaryDetailsHtml(dictionary, srcLang, targetLang) {
    ...
    const examples = [sense]
      .flatMap((sense) => (Array.isArray(sense.examples) ? sense.examples.slice(0, 4) : []))
      .filter((example) => {
        if (!example || typeof example.source !== "string" || typeof example.target !== "string")
          return false;
  ...
  async function handleSaveExampleClick(btn) {
    ...
    try {
      ...
      if (!translated) {
        const result = await SharedTranslatorService.translate(
          original,
          btn.dataset.tgtLang,
          btn.dataset.srcLang,
        );
  ...
  function attachTooltipHandlers() {
    ...
    if (exampleButtons.length) {
      SharedWordRepository.getStoredWords()
        .then((words) => {
          for (const btn of exampleButtons) {
            if (
              !words.some(
                (w) =>
                  w.original === cleanCardText(btn.dataset.src) &&
                  w.srcLang === btn.dataset.srcLang &&
                  w.tgtLang === btn.dataset.tgtLang,
              )
            )
              continue;
            btn.innerHTML = SVG.SAVE;
